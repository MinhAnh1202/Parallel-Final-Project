%%writefile verify_gpuOpt2_cpu_layers.cu
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
#include "gpu_layers_opt2.h"       // chứa các kernel GPU + CHECK_CUDA
...
void test_conv2d()
{
    ...
    conv2d_backward_weight_opt2<<<gridW, blockW>>>(
        d_x, d_dy, d_dw,
        N, C_in, H, W,
        C_out, pad, stride);
    ...
    conv2d_backward_bias_opt2<<<gridB, blockB, shmem_size>>>(
        d_dy, d_db,
        N, C_out, H_out, W_out);
